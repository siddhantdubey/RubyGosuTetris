class Matrix
    ...
    def look(value)
      ...
      @rowsarray.each_with_index do |rows, y|
    ...
    def trim
      ...
      @rowsarray = @rowsarray.drop_while { |rows| rows.all?(&:nil?) }
    ...
    def fixorientation
      ...
      @rowsarray.map! { |rows| Array(rows) }
      @rowsarray.map! { |rows| rows.concat([nil] * (width - rows.length)) }
    end
    ...
    def empty_columns
      ...
      @rowsarray.map { |rows| rows.find_index { |v| v } }.compact.min
    end
  end